...
namespace GravityKit\GravityRevisions\Foundation\Components;

use GravityKit\GravityRevisions\Foundation\Licenses\Framework;
use GravityKit\GravityRevisions\Foundation\State\StateManager;
use GravityKit\GravityRevisions\Foundation\State\UserStateManager;
	...
	public function ajax_newsletter_signup( array $payload ): array {
		...
		if ( ! is_email( $email ) ) {
			throw new InvalidArgumentException(
				esc_html__( 'The provided email address is not valid.', 'gk-gravityrevisions' )
			);
		}
		...
		if ( 401 === wp_remote_retrieve_response_code( $result ) ) {
			throw new RuntimeException( __( 'Something went wrong on our end. Please try again later.', 'gk-gravityrevisions' ) );
		}
