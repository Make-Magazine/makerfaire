...
class LicenseManager {
	...
	public function get_license_key_status_message( $status ) {
		$statuses = [
			'site_inactive'       => esc_html__( 'The license key is valid, but it has not been activated for this site.', 'gk-gravityrevisions' ),
			'inactive'            => esc_html__( 'The license key is valid, but it has not been activated for this site.', 'gk-gravityrevisions' ),
			'no_activations_left' => esc_html__( 'This license has reached its activation limit.', 'gk-gravityrevisions' ),
			'deactivated'         => esc_html__( 'This license has been deactivated.', 'gk-gravityrevisions' ),
			'valid'               => esc_html__( 'This license key is valid and active.', 'gk-gravityrevisions' ),
			'invalid'             => esc_html__( 'This license key is invalid.', 'gk-gravityrevisions' ),
			'missing'             => esc_html__( 'This license key is invalid.', 'gk-gravityrevisions' ),
			'revoked'             => esc_html__( 'This license key has been revoked.', 'gk-gravityrevisions' ),
			'expired'             => esc_html__( 'This license key has expired.', 'gk-gravityrevisions' ),
		];

		if ( empty( $statuses[ $status ] ) ) {
			LoggerFramework::get_instance()->warning( 'Unknown license status: ' . $status );

			return esc_html__( 'License status could not be determined.', 'gk-gravityrevisions' );
		}
	...
	public function perform_remote_license_call( $license, $edd_action ) {
		...
		foreach ( (array) $response as $key => $data ) {
			if ( ! isset( $data['success'] ) || ! isset( $data['license'] ) || ! isset( $data['checksum'] ) ) {
				throw new Exception( esc_html__( 'License data received from the API is incomplete.', 'gk-gravityrevisions' ) );
			}
	...
	public function ajax_activate_license( array $payload ) {
		...
		if ( empty( $payload['key'] ) ) {
			throw new Exception( esc_html__( 'Missing license key.', 'gk-gravityrevisions' ) );
		}
	...
	public function activate_license( $license_key ) {
		...
		if ( isset( $licenses_data[ $license_key ] ) ) {
			throw new Exception( esc_html__( 'This license is already activated.', 'gk-gravityrevisions' ) );
		}
	...
	public function ajax_deactivate_license( array $payload ) {
		...
		if ( empty( $licenses_data[ $license_key ] ) ) {
			throw new Exception( esc_html__( 'The license key is invalid.', 'gk-gravityrevisions' ) );
		}
		...
		try {
			...
			if ( ! $force_removal && ! Arr::get( $response, '_raw.success' ) ) {
				...
				if ( ! Arr::get( $response, 'expiry' ) || ! $this->is_expired_license( Arr::get( $response, 'expiry' ) ) ) {
					throw new Exception( esc_html__( 'Failed to deactivate license.', 'gk-gravityrevisions' ) );
				}
			}
		} catch ( Exception $e ) {
			if ( ! $force_removal ) {
	...
	public function modify_license_data_for_frontend_output( $license ) {
		...
		if ( preg_match( '/[^a-z]/i', $expiry ) ) {
			...
			$expiry = $expired
				? human_time_diff( $expiry, current_time( 'timestamp' ) ) . ' ' . esc_html_x( 'ago', 'Indicates "time ago"', 'gk-gravityrevisions' ) // phpcs:ignore WordPress.DateTime.CurrentTimeTimestamp
