...
namespace GravityKit\GravityRevisions\Foundation\Licenses;

use Exception;
use GravityKit\GravityRevisions\Foundation\Core;
use GravityKit\GravityRevisions\Foundation\Helpers\Arr;
use GravityKit\GravityRevisions\Foundation\Helpers\WP;
...
use GravityKit\GravityRevisions\Foundation\Licenses\WP\WPUpgraderSkin;
use Plugin_Upgrader;
use stdClass;
...
class ProductManager {
	...
	public function ajax_install_product( array $payload ): array {
		...
		if ( ! Framework::get_instance()->current_user_can( 'install_products' ) ) {
			throw new Exception( esc_html__( 'You do not have a permission to perform this action.', 'gk-gravityrevisions' ) );
		}
		...
		if ( ! $product ) {
			throw new Exception(
				strtr(
					esc_html_x( "Product with '[text_domain]' text domain not found.", 'Placeholders inside [] are not to be translated.', 'gk-gravityrevisions' ),
	...
	public function install_product( array $product ) {
		if ( ! file_exists( ABSPATH . 'wp-admin/includes/class-wp-upgrader.php' ) ) {
			throw new Exception( esc_html__( 'Unable to load core WordPress files required to install the product.', 'gk-gravityrevisions' ) );
		}
		...
		if ( ! $product_download_link ) {
			throw new Exception( esc_html__( 'Unable to locate product download link.', 'gk-gravityrevisions' ) );
		}
		...
		try {
			...
			$error = join(
				' ',
				[
					esc_html__( 'Installation failed.', 'gk-gravityrevisions' ),
					$e->getMessage(),
				]
			);
		...
		if ( ! $installer->result ) {
			throw new Exception( esc_html__( 'Installation failed.', 'gk-gravityrevisions' ) );
		}
	}
	...
	public function update_product( array $product ) {
		...
		try {
			...
			$error = join(
				' ',
				[
					esc_html__( 'Update failed.', 'gk-gravityrevisions' ),
	...
	public function ajax_delete_product( array $payload ): array {
		...
		if ( $product['active'] ) {
			throw new Exception(
				esc_html__( 'Product must be deactivated before it can be deleted.', 'gk-gravityrevisions' )
			);
		}
	...
	public function delete_product( array $product ) {
		...
		if ( is_null( $result ) ) {
			throw new Exception( esc_html__( 'Could not delete the product due to missing filesystem credentials.', 'gk-gravityrevisions' ) );
		}
	}
	...
	public function activate_product( array $product ) {
		if ( $this->is_product_active_in_current_context( $product['path'] ) ) {
			throw new Exception( esc_html__( 'Product is already active.', 'gk-gravityrevisions' ) );
		}
		...
		if ( is_wp_error( $result ) ) {
			throw new Exception(
				strtr(
					esc_html_x( 'Could not activate the product. [error]', 'Placeholders inside [] are not to be translated.', 'gk-gravityrevisions' ),
        ...
        return [
            ...
            'ui_action' => [
                ...
                'redirect' => ! $backend_foundation_version ? [
                    ...
                    'loader_title'   => esc_html__( 'Redirecting to the Plugins page…', 'gk-gravityrevisions' ),
                    'loader_message' => esc_html__( 'Manage Your Kit functionality is no longer available.', 'gk-gravityrevisions' ),
                ] : false,
            ],
        ];
	}
	...
	public function deactivate_product( array $product ) {
		if ( ! $this->is_product_active_in_current_context( $product['path'] ) ) {
			throw new Exception( esc_html__( 'Product in not active.', 'gk-gravityrevisions' ) );
		}

		deactivate_plugins( $product['path'], false, CoreHelpers::is_network_admin() );

		if ( $this->is_product_active_in_current_context( $product['path'] ) ) {
			throw new Exception( esc_html__( 'Could not deactivate the product.', 'gk-gravityrevisions' ) );
		}
	}
	...
	public function get_remote_products() {
		...
		if ( empty( $response ) || empty( $products ) ) {
			throw new Exception( esc_html__( 'Invalid product information received from the API.', 'gk-gravityrevisions' ) );
		}
	...
	public function truncate_product_changelog( $changelog, $product_url, $max_changelog_entries = 3, $link_to_full_changelog = true ) {
		...
		if ( count( $parsed_changelog[0] ) > count( $truncated_changelog ) && $link_to_full_changelog ) {
			$truncated_changelog[] = sprintf(
				...
				$product_url,
				esc_html__( 'View full changelog', 'gk-gravityrevisions' )
			);
		}
