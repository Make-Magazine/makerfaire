...
namespace GravityKit\GravityRevisions\Foundation\Licenses\WP;

use GravityKit\GravityRevisions\Foundation\Core;
use GravityKit\GravityRevisions\Foundation\Helpers\Arr;
use GravityKit\GravityRevisions\Foundation\Licenses\Framework;
use GravityKit\GravityRevisions\Foundation\Settings\Framework as SettingsFramework;
use GravityKit\GravityRevisions\Foundation\Licenses\ProductManager;
use GravityKit\GravityRevisions\Foundation\Licenses\LicenseManager;
...
JS;
	...
	public function modify_product_action_links( $links, $plugin_path, $plugin_data ) {
		...
		if ( isset( $plugin_data['GravityKitGroup'] ) ) {
			return [
				'manage'           => sprintf(
					...
					esc_html__( 'Manage Your Kit', 'gk-gravityrevisions' )
				),
				'settings'         => sprintf(
					'<a href="%s">%s</a>',
					esc_url_raw( add_query_arg( [ 'page' => SettingsFramework::ID ], admin_url( 'admin.php' ) ) ),
					esc_html__( 'Settings', 'gk-gravityrevisions' )
				),
				'disable_grouping' => sprintf(
					'<a href="%s" title="%s">%s</a>',
					esc_url_raw( add_query_arg( [ 'gk_disable_grouping' => 1 ], admin_url( 'plugins.php' ) ) ),
					esc_attr__( 'Disable the grouping of GravityKit products', 'gk-gravityrevisions' ),
					esc_html__( 'Ungroup', 'gk-gravityrevisions' )
				),
			];
		}
		...
		if ( $product ) {
				...
				*/
				...
				if ( ! $product['checked_dependencies'][ $product['installed_version'] ]['status'] ?? false ) {
					$links['activate'] = sprintf(
						...
						esc_html__( 'This product has unmet dependencies. Click this link to see see what they are.', 'gk-gravityrevisions' ),
						esc_html__( 'Activate…', 'gk-gravityrevisions' )
					);
				}
				...
				if ( $product['has_git_folder'] && isset( $links['delete'] ) ) {
					...
					if ( $deletion_link ) {
						$links['delete'] = sprintf(
							...
							$deletion_link,
							strtr(
								esc_html_x( '[product] is installed from a Git repository. Click this link to confirm deletion.', 'Placeholders inside [] are not to be translated.', 'gk-gravityrevisions' ),
								[ '[product]' => $product['name'] ]
							),
							strtr(
								esc_html_x( '[product] is installed from a Git repository. Are you sure you want to delete it?', 'Placeholders inside [] are not to be translated.', 'gk-gravityrevisions' ),
								[ '[product]' => $product['name'] ]
							),
							esc_html__( 'Delete…', 'gk-gravityrevisions' )
						);
					}
				}
			}
			...
			if ( $product['active'] && ! empty( $product['required_by'] ) && isset( $links['deactivate'] ) ) {
				...
				if ( $deactivation_link ) {
					...
					$links['deactivate'] = sprintf(
						...
						$deactivation_link,
						strtr(
							esc_html_x( '[product] is required by other products to be active. Click this link to see which ones and to confirm deactivation.', 'Placeholders inside [] are not to be translated.', 'gk-gravityrevisions' ),
							[ '[product]' => $product['name'] ]
						),
						strtr(
							esc_html_x( '[product] is required by [products] to be active. Are you sure you want to deactivate it?', 'Placeholders inside [] are not to be translated.', 'gk-gravityrevisions' ),
							[
								'[product]'  => $product['name'],
								'[products]' => $required_by,
							]
						),
						esc_html__( 'Deactivate…', 'gk-gravityrevisions' )
					);
				}
			}
			...
			$gk_links['support'] = sprintf(
				...
				esc_html__( 'Support', 'gk-gravityrevisions' )
			);
		}
		...
		if ( ( $product && count( $products ) > 1 ) || ( count( $products ) && $plugin_data && $plugin_data['TextDomain'] === $foundation_info['source_plugin']['TextDomain'] ) ) {
			$gk_links['enable_grouping'] = sprintf(
				...
				esc_html__( 'Aggregate all GravityKit products into a single entry on the Plugins page for a cleaner view and easier management.', 'gk-gravityrevisions' ),
				esc_html__( 'Group', 'gk-gravityrevisions' )
			);
		}
	...
	public function group_products( $wp_plugins ) {
			...
			foreach ( $wp_plugins as $path => &$wp_plugin ) {
				...
				if ( $wp_plugin['TextDomain'] === $foundation_info['source_plugin']['TextDomain'] ) {
					...
					$wp_plugin = array_merge(
						$wp_plugin,
						[
							'Name'            => __( 'GravityKit', 'gk-gravityrevisions' ),
		...
		add_filter(
			...
			function ( $wp_plugin_meta, $wp_plugin_file, $wp_plugin_data ) {
				...
				return [
					'<a href="https://www.gravitykit.com">' . esc_html__( 'Visit GravityKit.com', 'gk-gravityrevisions' ) . '</a>',
				];
			},
			10,
			4
	...
	public function enqueue_update_notices( $plugin_path, $plugin_data ) {
		...
		if ( $this->should_group_products() ) {
			...
			if ( ! $product['checked_dependencies'][ $product['installed_version'] ]['status'] ?? false ) {
				$notice = strtr(
					esc_html_x( 'There is a new version [version] of [product] available. [link]Update now…[/link].', 'Placeholders inside [] are not to be translated.', 'gk-gravityrevisions' ),
					[
						'[product]' => $product['name'],
						...
						'[link]'    => sprintf(
							...
							esc_attr__( 'This product has unmet dependencies. Click this link to see see what they are.', 'gk-gravityrevisions' )
						),
						'[/link]'   => '</a>',
					]
				);
	...
	public function enqueue_unlicensed_notices( $plugin_path, $plugin_data ) {
		...
		if ( isset( $plugin_data['GravityKitGroup'] ) ) {
			...
			$notice = strtr(
				esc_html_x( 'This is an unlicensed product. Please [link]visit the licensing page[/link] to enter a valid license or to purchase a new one.', 'Placeholders inside [] are not to be translated.', 'gk-gravityrevisions' ),
				[
