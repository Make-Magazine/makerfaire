...
class GV_Extension_DataTables_Data {
	...
	function get_datatables_data() {
		...
		if ( $view = $view_collection->get( $view_id ) ) {
			...
			gravityview()->log->debug( 'View #{view_id} found in $post View Collection', array( 'view_id' => $view_id, 'data' => $atts ) );
	...
	public function get_datatables_script_configuration( $post, $view ) {
		...
		$dt_config =  array(
			...
			'stateSave'	 => gravityview()->request->is_search() ? false : true, // On refresh (and on single entry view, then clicking "go back"), save the page you were on.
			...
			'lengthMenu'	=> $this->get_length_menu( $view ), // Dropdown pagination length menu
