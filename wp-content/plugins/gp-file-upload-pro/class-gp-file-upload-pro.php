...
class GP_File_Upload_Pro extends GP_Plugin {
	...
	public function get_rehydration_info( $field, $value ) {
		...
		// Handle rehydration if using GP Media Library.
		if ( is_numeric( $value ) && wp_get_attachment_url( $value ) ) {
			...
			/*
			 * Populate GFFormsModel::$uploaded_files[ $form_id ][ "input_{$field->id}" ] that way if the user doesn't
			 * change the file, Gravity Forms will not throw a validation error.
			 */
			if ( ! isset( GFFormsModel::$uploaded_files[ $form_id ] ) ) {
				GFFormsModel::$uploaded_files[ $form_id ] = array();
			}

			$uploaded_filename = basename( $path );

			GFFormsModel::$uploaded_files[ $form_id ][ "input_{$field->id}" ] = array(
				array(
