...
jQuery(document).ready(function($){
	...
	/**
	 * Show the custom image-map uploader only if the map mode is image
	 */
	maptiles_uploader_display_toggle();
	 ...
	 * Show the custom image-map uploader only if the map mode is image
	 */
	function maptiles_uploader_display_toggle() {
		const mode = $( '#acf-osm-map-mode' ).val();
		const image_map_uploader = "#acf-mapify_acf_group_6049938682e7f";
		const map_style_selector = "#acf-mapify_acf_group_6066bc3dd3999";
		
		if ( 'image' === mode ) {
			$( image_map_uploader ).show();
			$( map_style_selector ).hide();
		} else {
			$( image_map_uploader ).hide();
			$( map_style_selector ).show();
		}
	}
