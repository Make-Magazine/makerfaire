...
class Mpfy_Map {
	...
	function __construct( $map_id ) {
		$this->post = get_post( intval( $map_id ) );
	...
	function get_tooltip_image_orientation() {
		$value = get_post_meta( $this->get_id(), '_map_tooltip_image_orientation', true );
		$value = $value ? $value : 'left';
		return apply_filters( 'mpfy_map_tooltip_image_orientation', $value, $this->get_id() );
	}
	...
	function get_type() {
		$value = apply_filters( 'mpfy_map_type', $this->get_id() );
		return $value;
	}
	...
	function get_search_radius_unit_name() {
		...
		$value = ( 'km' === $unit ) ? __( 'Kilometers', 'mpfy' ) : __( 'Miles', 'mpfy');
		return $value;
	}
	...
	function get_search_radius() {
		$front_end_enabled = get_post_meta( $this->get_id(), '_map_enable_search_radius', true );

		if ( $front_end_enabled === 'yes' ) {
			$value = $this->get_search_radius_options();
			$value = max( 1, intval( $value[0] ) );
		} else {
	...
	function get_location_circle_color() {
		$meta  = get_post_meta( $this->get_id(), '_map_location_circle_color', true );
		$value = empty( $meta ) ? '#3388ff' : $meta;
		return apply_filters( 'mpfy_map_location_circle_color', $value, $this->get_id() );
	}
	...
	function get_crowdmap_enabled() {
		$value = mpfy_meta_to_bool($this->get_id(), '_map_crowd_enabled', false);
		return $value;
	}
}
