...
abstract class Mpfy_Carbon_Video {
	...
	static function create($video_code) {
		...
		$video_providers = array("Youtube", "Vimeo", "Facebook");
 ...
 */
class Mpfy_Carbon_Video_Facebook extends Mpfy_Carbon_Video {

	/**
	 * Check whether video code looks remotely like facebook link, short link or embed code. 
	 * Returning true here doesn't guarantee that the code will be actually paraseable. 
	 * 
	 * @param  string $video_code
	 * @return boolean
	 */
	static function test($video_code) {
		return preg_match('~(https?:)?//(www\.)?(facebook\.com)~i', $video_code);
	}
	...
	public function parse($video_code) {
		...
		if (preg_match($iframe_pattern, $video_code)) {
			if (preg_match('/href=([^&]+)/', $video_code, $matches)) {
				$hrefValue = trim($matches[1], " '\""); // Removing extra characters
				$this->fb_video_url = urldecode($hrefValue); // Decoding URL-encoded string
			} else {
				return false;
			}
		} else {
			$this->fb_video_url = $video_code;
		}
	...
	public function get_embed_code($width=null, $height=null) {
		$width = $this->get_embed_width($width);
		$height = $this->get_embed_height($height);
		$src = 'https://www.facebook.com/plugins/video.php?href=' . urlencode($this->fb_video_url) . '&show_text=false&t=0';
	...
	public function get_image() {
		$width = 104;
		$height = 60;
		$src = 'https://www.facebook.com/plugins/video.php?href=' . urlencode($this->fb_video_url) . '&show_text=false&t=0&width=' . $width . '&height=' . $height;

		return '<iframe width="' . $width . '" height="' . $height . '" src="' . $src . '" frameborder="0"></iframe>';
	}
...
/**
 * Facebook OLD handling code. 
 */
class Mpfy_Carbon_Video_FacebookOLD extends Mpfy_Carbon_Video {
	protected $default_width = '640';
	protected $default_height = '360';

	/**
	 * The default domain name for facebook videos
	 */
	const DEFAULT_DOMAIN = 'www.facebook.com';

	/**
	 * The original domain name of the video
	 * @var string
	 */
	public $domain = self::DEFAULT_DOMAIN;
	...
	function parse($video_code) {
		$regexes = array(
			// Something like: https://www.facebook.com/OppenheimerMovie/videos/3102330160156415
			"url_regex" =>
				'~^' . 
					$this->regex_fragments['protocol'] . 
					'facebook\.com/.*?/?' . 
			...
			// <iframe src="https://www.facebook.com/plugins/video.php?height=314&href=https%3A%2F%2Fwww.facebook.com%2FOppenheimerMovie%2Fvideos%2F3102330160156415%2F&show_text=false&width=560&t=0" width="560" height="314" style="border:none;overflow:hidden" scrolling="no" frameborder="0" allowfullscreen="true" allow="autoplay; clipboard-write; encrypted-media; picture-in-picture; web-share" allowFullScreen="true"></iframe>
			"embed_code_regex" =>
				'~'.
					'<iframe.*?src=[\'"]' .
					$this->regex_fragments['protocol'] . 
					'(?P<domain>(www\.)?facebook\.com/plugins/video\.php\?.*?href=https%3A%2F%2Fwww\.facebook\.com%2F[^\'"&]+%2Fvideos%2F' .
					$this->regex_fragments['video_id'] .
				'~i',
		);
