	...
	class WC_AM_Client_25 {
		...
		public function __construct( $file, $product_id, $software_version, $plugin_or_theme, $api_url, $software_title = '', $text_domain = '' ) {
			...
			if ( is_admin() ) {
				...
				$this->wc_am_settings_menu_title         = $this->software_title . esc_html__( ' Verification', $this->text_domain );
				$this->wc_am_settings_title              = $this->software_title . esc_html__( ' API Key Verification', $this->text_domain );
				$this->wc_am_menu_tab_activation_title   = esc_html__( 'API Key Verification', $this->text_domain );
		...
		public function inactive_notice() { ?>
			...
			<?php if ( isset( $_GET[ 'page' ] ) && $this->wc_am_activation_tab_key == sanitize_text_field( $_GET[ 'page' ] ) ) {
				return;
			} ?>
            <div class="notice notice-error mapifypro-notice">
                <p><?php 
				printf( __( 'The MapifyPro API Key has not been verified, so your maps will not display! %sClick here to verify MapifyPro%s. If you have already done this, there may be an issue with your MapifyPro subscription. Please check your MapifyPro account at %s', $this->text_domain ), 
					'<a href="' . esc_url( admin_url( 'options-general.php?page=' . $this->wc_am_activation_tab_key ) ) . '">', '</a>',
					'<a href="https://mapifypro.com/my-account/subscriptions/" target="blank">https://mapifypro.com/my-account/subscriptions/</a>'
				); ?></p>
            </div>
			<?php
		}
		...
		public function config_page() {
			...
			$current_tab   = isset( $_GET[ 'tab' ] ) ? sanitize_text_field( $_GET[ 'tab' ] ) : $this->wc_am_activation_tab_key;
			$tab           = isset( $_GET[ 'tab' ] ) ? sanitize_text_field( $_GET[ 'tab' ] ) : $this->wc_am_activation_tab_key;
			?>
            <div class='wrap'>
                <h1 style="font-weight: 600; margin-bottom: 10px;"><?php esc_html_e( $this->wc_am_settings_title, $this->text_domain ); ?></h1>
		...
		public function load_settings() {
			...
			// API Key
			add_settings_section( $this->wc_am_api_key_key, esc_html__( 'API Key Verification', $this->text_domain ), array(
				$this,
				'wc_am_api_key_text'
			...
			add_settings_field( 'status', esc_html__( 'API Key Status', $this->text_domain ), array(
				$this,
				'wc_am_api_key_verification_status'
			), $this->wc_am_activation_tab_key, $this->wc_am_api_key_key );
		...
		public function wc_am_api_key_status() {
			...
			if ( ! empty( $license_status[ 'status_check' ] ) && $license_status[ 'status_check' ] == 'active' ) {
		...
		// The alias of the function wc_am_api_key_status, but this will return "Verified" or "Not Verified" instead
		public function wc_am_api_key_verification_status() {
			$license_status = $this->license_key_status();
			
			if ( ! empty( $license_status[ 'status_check' ] ) && $license_status[ 'status_check' ] == 'active' ) {
				$license_status_check = esc_html__( 'Verified', $this->text_domain );
				update_option( $this->wc_am_activated_key, 'Activated' );
				update_option( $this->wc_am_deactivate_checkbox_key, 'off' );
			} else {
				$license_status_check = esc_html__( 'Not Verified', $this->text_domain );
			}
			...
			if ( $_REQUEST[ 'option_page' ] != $this->wc_am_deactivate_checkbox_key ) {
				if ( $activation_status == 'Deactivated' || $activation_status == '' || $api_key == '' || $checkbox_status == 'on' || $current_api_key != $api_key ) {
					...
					if ( $activate_results[ 'success' ] === true && $activate_results[ 'activated' ] === true ) {
						add_settings_error( 'activate_text', 'activate_msg', sprintf( __( '%s verified. ', $this->text_domain ), esc_attr( $this->software_title ) ) . esc_attr( "{$activate_results['message']}." ), 'updated' );
						update_option( $this->wc_am_activated_key, 'Activated' );
						update_option( $this->wc_am_deactivate_checkbox_key, 'off' );
						update_option( 'mpfy_plugin_license_version', $this->software_version ); // update license version
					}

					if ( $activate_results == false && ! empty( $this->data ) && ! empty( $this->wc_am_activated_key ) ) {
						add_settings_error( 'api_key_check_text', 'api_key_check_error', esc_html__( 'Connection failed to the License Key API server. Try again later. There may be a problem on your server preventing outgoing requests, or the store is blocking your request to verify the API key.', $this->text_domain ), 'error' );
		...
		public function replace_license_key( $current_api_key ) {
			...
			add_settings_error( 'not_deactivated_text', 'not_deactivated_error', esc_html__( 'The API Key could not be deactivated. Use the API Key Deactivation tab to manually deactivate the API Key before verifying a new API Key. If all else fails, go to Plugins, then deactivate and reactivate this plugin, or if a theme change themes, then change back to this theme, then go to the Settings for this plugin/theme and enter the API Key information again to verify it. Also check the My Account dashboard to see if the API Key for this site was still active before the error message was displayed.', $this->text_domain ), 'updated' );
		...
		public function wc_am_deactivate_textarea() {
			...
			echo '/>';
			?><label class="description" for="<?php echo esc_attr( $this->wc_am_deactivate_checkbox_key ) ?>">
