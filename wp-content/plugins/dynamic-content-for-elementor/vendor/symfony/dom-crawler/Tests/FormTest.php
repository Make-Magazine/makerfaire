 ...
 */
namespace DynamicOOOS\Symfony\Component\DomCrawler\Tests;

use DynamicOOOS\PHPUnit\Framework\TestCase;
use DynamicOOOS\Symfony\Component\DomCrawler\Field\TextareaFormField;
use DynamicOOOS\Symfony\Component\DomCrawler\Form;
use DynamicOOOS\Symfony\Component\DomCrawler\FormFieldRegistry;
class FormTest extends TestCase
{
    public static function setUpBeforeClass()
    {
        // Ensure that the private helper class FormFieldRegistry is loaded
        \class_exists('DynamicOOOS\\Symfony\\Component\\DomCrawler\\Form');
    }
    public function testConstructorThrowsExceptionIfTheNodeHasNoFormAncestor()
    {
        ...
        try {
            ...
            $this->assertTrue(\true, '__construct() throws a \\LogicException if the node has no form ancestor');
        }
        try {
            ...
            $this->assertTrue(\true, '__construct() throws a \\LogicException if the input type is not submit, button, or image');
        }
    ...
    public function constructorThrowsExceptionIfNoRelatedFormProvider()
    {
        ...
        ');
        $nodes = $dom->getElementsByTagName('input');
        return [[$nodes->item(0)], [$nodes->item(1)]];
    }
    ...
    public function testConstructorHandlesFormValues()
    {
        ...
        $values1 = ['apples' => ['1', '2'], 'form_name' => 'form-1', 'button_1' => 'Capture fields', 'outer_field' => 'success'];
        $values2 = ['oranges' => ['1', '2', '3'], 'form_name' => 'form_2', 'button_2' => '', 'app_frontend_form_type_contact_form_type' => ['contactType' => '', 'firstName' => 'John']];
    ...
    public function testMultiValuedFields()
    {
        ...
        ');
        $this->assertEquals(['foo[2]', 'foo[3]', 'bar[foo][0]', 'bar[foo][foobar]'], \array_keys($form->all()));
        ...
        $this->assertEquals('foobar', $form->get('bar[foo][foobar]')->getValue());
    }
    /**
    ...
    {
        return [['does not take into account input fields without a name attribute', '<input type="text" value="foo" />
                 <input type="submit" />', []], ['does not take into account input fields with an empty name attribute value', '<input type="text" name="" value="foo" />
                 <input type="submit" />', []], ['takes into account disabled input fields', '<input type="text" name="foo" value="foo" disabled="disabled" />
                 <input type="submit" />', ['foo' => ['InputFormField', 'foo']]], ['appends the submitted button value', '<input type="submit" name="bar" value="bar" />', ['bar' => ['InputFormField', 'bar']]], ['appends the submitted button value for Button element', '<button type="submit" name="bar" value="bar">Bar</button>', ['bar' => ['InputFormField', 'bar']]], ['appends the submitted button value but not other submit buttons', '<input type="submit" name="bar" value="bar" />
                 <input type="submit" name="foobar" value="foobar" />', ['foobar' => ['InputFormField', 'foobar']]], ['turns an image input into x and y fields', '<input type="image" name="bar" />', ['bar.x' => ['InputFormField', '0'], 'bar.y' => ['InputFormField', '0']]], ['returns textareas', '<textarea name="foo">foo</textarea>
                 <input type="submit" />', ['foo' => ['TextareaFormField', 'foo']]], ['returns inputs', '<input type="text" name="foo" value="foo" />
                 <input type="submit" />', ['foo' => ['InputFormField', 'foo']]], ['returns checkboxes', '<input type="checkbox" name="foo" value="foo" checked="checked" />
                 <input type="submit" />', ['foo' => ['ChoiceFormField', 'foo']]], ['returns not-checked checkboxes', '<input type="checkbox" name="foo" value="foo" />
                 <input type="submit" />', ['foo' => ['ChoiceFormField', \false]]], ['returns radio buttons', '<input type="radio" name="foo" value="foo" />
                 <input type="radio" name="foo" value="bar" checked="bar" />
                 <input type="submit" />', ['foo' => ['ChoiceFormField', 'bar']]], ['returns file inputs', '<input type="file" name="foo" />
                 <input type="submit" />', ['foo' => ['FileFormField', ['name' => '', 'type' => '', 'tmp_name' => '', 'error' => 4, 'size' => 0]]]]];
    }
    ...
    public function testGetSetValue()
    {
        ...
        try {
            ...
            $this->assertTrue(\true, '->offsetSet() throws an \\InvalidArgumentException exception if the field does not exist');
        }
        try {
            $form['foobar'];
            $this->fail('->offsetSet() throws an \\InvalidArgumentException exception if the field does not exist');
    ...
    {
        return [['returns the URI of the form', '<form action="/foo"><input type="submit" /></form>', [], '/foo'], ['appends the form values if the method is get', '<form action="/foo"><input type="text" name="foo" value="foo" /><input type="submit" /></form>', [], '/foo?foo=foo'], ['appends the form values and merges the submitted values', '<form action="/foo"><input type="text" name="foo" value="foo" /><input type="submit" /></form>', ['foo' => 'bar'], '/foo?foo=bar'], ['does not append values if the method is post', '<form action="/foo" method="post"><input type="text" name="foo" value="foo" /><input type="submit" /></form>', [], '/foo'], ['does not append values if the method is patch', '<form action="/foo" method="post"><input type="text" name="foo" value="foo" /><input type="submit" /></form>', [], '/foo', 'PUT'], ['does not append values if the method is delete', '<form action="/foo" method="post"><input type="text" name="foo" value="foo" /><input type="submit" /></form>', [], '/foo', 'DELETE'], ['does not append values if the method is put', '<form action="/foo" method="post"><input type="text" name="foo" value="foo" /><input type="submit" /></form>', [], '/foo', 'PATCH'], ['appends the form values to an existing query string', '<form action="/foo?bar=bar"><input type="text" name="foo" value="foo" /><input type="submit" /></form>', [], '/foo?bar=bar&foo=foo'], ['replaces query values with the form values', '<form action="/foo?bar=bar"><input type="text" name="bar" value="foo" /><input type="submit" /></form>', [], '/foo?bar=foo'], ['returns an empty URI if the action is empty', '<form><input type="submit" /></form>', [], '/'], ['appends the form values even if the action is empty', '<form><input type="text" name="foo" value="foo" /><input type="submit" /></form>', [], '/?foo=foo'], ['chooses the path if the action attribute value is a sharp (#)', '<form action="#" method="post"><input type="text" name="foo" value="foo" /><input type="submit" /></form>', [], '/#']];
    }
    ...
    {
        $form = $this->createForm('<form method="post"><input type="text" name="bar" value="bar" /><input type="submit" /></form>');
        $this->assertInstanceOf('DynamicOOOS\\Symfony\\Component\\DomCrawler\\Field\\InputFormField', $form->get('bar'), '->get() returns the field object associated with the given name');
        try {
            $form->get('foo');
            $this->fail('->get() throws an \\InvalidArgumentException if the field does not exist');
        } catch (\InvalidArgumentException $e) {
            $this->assertTrue(\true, '->get() throws an \\InvalidArgumentException if the field does not exist');
        }
    }
    public function testAll()
    {
        ...
        $this->assertInstanceOf('DynamicOOOS\\Symfony\\Component\\DomCrawler\\Field\\InputFormField', $fields['bar'], '->all() return an array of form field objects');
    }
    ...
    public function testFormRegistrySetValues()
    {
        ...
        $registry->set('foo', [2 => 2, 3 => 3, 'bar' => ['baz' => 'fbb']]);
    }
    ...
    public function testDifferentFieldTypesWithSameName()
    {
        ...
        ');
        $form = new Form($dom->getElementsByTagName('form')->item(0), 'http://example.com');
        $this->assertInstanceOf('DynamicOOOS\\Symfony\\Component\\DomCrawler\\Field\\ChoiceFormField', $form->get('option'));
    }
    protected function getFormFieldMock($name, $value = null)
    {
        $field = $this->getMockBuilder('DynamicOOOS\\Symfony\\Component\\DomCrawler\\Field\\FormField')->setMethods(['getName', 'getValue', 'setValue', 'initialize'])->disableOriginalConstructor()->getMock();
        $field->expects($this->any())->method('getName')->willReturn($name);
        $field->expects($this->any())->method('getValue')->willReturn($value);
        return $field;
    }
    ...
    public function testGetReturnTypes()
    {
        ...
        // Array of FormField
        $this->assertSame(['bar' => $textareaFormField], $form->get('foo[collection][0]'));
        // Array of array of FormField
        $this->assertSame([['bar' => $textareaFormField]], $form->get('foo[collection]'));
    }
}
