jQuery(document).ready(function($) {
    ...
    function sbiAfterUpdateToken(savedToken,saveID){
        ...
        if ($('#sbi_connected_account_'+savedToken.user_id).length) {
            ...
            var removeOrSaveHTML = saveID ? '<a href="JavaScript:void(0);" class="sbi_remove_from_user_feed button-primary"><i class="fa fa-minus-circle" aria-hidden="true"></i>Remove from Primary Feed</a>' : '<a href="JavaScript:void(0);" class="sbi_use_in_user_feed button-primary"><i class="fa fa-plus-circle" aria-hidden="true"></i>Add to Primary Feed</a>',
                ...
                html = '<div class="sbi_connected_account '+statusClass+' sbi-init-click-remove" id="sbi_connected_account_'+savedToken.user_id+'" data-accesstoken="" data-userid="'+savedToken.user_id+'" data-username="'+savedToken.username+'"'+permissionsAtt+'>'+
    ...
    sbiToggleOrder();
    jQuery('input[name=sb_instagram_type]').on('change',function(){
        sbiToggleOrder();
    });
    ...
    jQuery('#sbi_admin #sb_instagram_moderation_mode').on('click',function(){
    ...
    jQuery('.sb_instagram_incex_one_all').on('click',function() {
    ...
    jQuery('.sb_instagram_moderation_mode').on('click',function() {
    ...
    jQuery('.sb_instagram_mobile_layout_reveal').on('click',function() {
    ...
    jQuery('.sbi_show_advanced_reveal').on('click',function() {
    ...
    $sbiClearWhiteListsButton.on('click',function(event) {
        event.preventDefault();
    ...
    $sbiClearPermanentWhiteListsButton.on('click',function(event) {
        event.preventDefault();
    ...
    $sbiClearCommentCacheButton.on('click',function(event) {
        event.preventDefault();
    ...
    jQuery('#sbi_admin th label, #sbi_admin .sb_instagram_layout_setting label, #sbi_admin .sb_instagram_box_setting label, #sbi_admin .sbi_shortcode_label').on('click',function(){
    ...
    jQuery('#sbi_admin .sbi_mixed_directions .sbi_col').on('click',function(){
        jQuery(this).parent().find('.sbi_tooltip').slideToggle();
    });

    //Shortcode label on hover
    jQuery('#sbi_admin th, #sbi_admin .sb_instagram_layout_setting label, #sbi_admin .sb_instagram_box_setting label, #sbi_admin .sbi_shortcode_label').on('mouseenter mouseleave', function(e) {
        switch(e.type) {
            case 'mouseenter':
    ...
    $('#sb_instagram_header_style').on('change',function(){
    ...
    jQuery('.sbi-redirect-disable').on('click',function(event) {
        event.preventDefault();
    ...
    jQuery('select[name=sb_instagram_media_type]').on('change', function() {
