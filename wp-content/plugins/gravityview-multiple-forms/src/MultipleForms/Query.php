...
class Query extends GF_Query {
	...
	public function parse( $form_id, $search_criteria = [], $sorting = null, $paging = null ) {
		...
		/**
		 * Configures which form and field we should look into for ordering.
		 */
		$order = Sort::instance()->configure_order_column( $sort_field, $form_id );
		...
		if ( ! empty( $property_conditions ) ) {
			if ( count( $property_conditions ) > 1 ) {
				$properties_condition = call_user_func_array( [ GF_Query_Condition::class, '_and' ], $property_conditions );
			} else {
		...
		if ( ! empty( $search_criteria['field_filters'] ) ) {
			...
			foreach ( $field_filters as $filter ) {
				...
				if ( 'is_approved' === $key ) {
					$approved_condition = new GF_Query_Condition(
						$column,
						$operator,
						$literal
					);
					$filters[] = apply_filters( 'gk/multiple-forms/query/search_criteria/is_approved_condition', $approved_condition, $search_criteria, $this );
	 ...
	 * @param string                        $order  The order (one of self::ASC, self::DESC or empty). Default for GF_Query_Column: self::ASC Default for
	 *                                              GF_Query_Call: empty
	 *
	 * @return GF_Query Chainable $this.
	 */
		...
		foreach ( $explicit_joins as $join ) {
			if ( preg_match( '#` AS `([motc]\d+)` ON #', $join, $matches ) ) {
				if ( $key = array_search( $matches[1], $this->aliases, true ) ) {
					[ $form_id, $field_id ] = explode( '_', $key );
				...
				if ( ! in_array( $matches[1], $explicit_join_aliases ) ) {
					if ( ( $key = array_search( $matches[1], $this->aliases, true ) ) !== false ) {
						...
						if ( isset( $explicit_join_aliases[ $form_id ] ) && strpos( $explicit_join_aliases[ $form_id ], 't' ) !== 0 ) {
							[ $table, $on ] = explode( ' ON ', $join );
							$join = implode( ' ON ', [
								$table,
								sprintf( '`%s`.`entry_id` = `%s`.`entry_id`', $matches[1], $explicit_join_aliases[ $form_id ] ),
							] );
						}
					}
					$remaining_inference_joins[] = $join;
				}
			} elseif ( preg_match( '#`(\d+)` ON `%s`.`id` = `%s`.`entry_id`$#', $join, $matches ) ) {
				if ( ( $alias = $explicit_join_aliases[ $matches[1] ] ) && 't' !== $alias[0] ) {
					[ $table, $as ] = explode( ' AS ', $join );
					$as                          = str_replace( $matches[1], $this->_alias( null, $matches[1] ), $as );
					$join                        = sprintf( '%s AS %s', $table, $as );
