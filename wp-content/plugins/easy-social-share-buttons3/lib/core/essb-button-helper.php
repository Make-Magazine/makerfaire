...
function essb_get_share_address($network, $share = array(), $salt = '', $amp_endpoint = false, $is_mobile = false) {
	...
	switch ($network) {
		...
		case 'copy':
			$url = '#';
			$api_command = 'essb.copy_link(&#39;' . $salt . '&#39;); return false;'; 
			break;
		case 'mix':
...
function essb_draw_buttons_start($style = array(), $position = '', $salt = '', $like_or_share = 'share', $share = array(), $cached_counters = array(), $networks = array()) {
	 ...
	 */
	if (essb_option_bool_value('activate_automatic_mobile_content') && ($position == 'top' || $position == 'bottom' || $position == 'float')) {
...
function essb_button_generate_flex_width($style) {
