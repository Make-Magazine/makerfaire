    ...
    <div v-if="showData" v-for="(faire, faire_name) in faire_entries" style="margin-bottom:50px">
      ...
      <b-card :id="entry.project_id" v-for="entry in faire.entries" :key="entry.project_id" style="margin-bottom:50px;">
        ...
        <b-row fluid>
          ...
          <b-col md="9" sm="12" class="entry-content">
            <b-row align-v="baseline" align-h="between">
              <b-col md="7" sm="12">
                <h3>{{entry.project_name}}</h3>
              </b-col>
              <b-col cols="6" md="3" sm="6"><span :class="'status_'+entry.project_id">{{entry.status}}</span></b-col>
              <b-col cols="6" md="2" sm="6" style="color: #ccc;text-align:right;">{{entry.project_id}}</b-col>
            </b-row>
            <b-row> 
              <b-col sm="6" cols="12">
                <div>
            ...
            <span class="last-row" v-if="Date.now() <= new Date(faire.faire_end_dt).getTime()">
                <div v-if="entry.links.length" v-for="link in entry.links">
                    <a :href="link.link" target="_blank" style="margin-right:10px;">{{link.title}}</a>
                ...
                <div align-h="between" v-if="entry.status!='Cancelled'" class="tasks-row"><!-- Tickets/Tasks/Manage Section-->
        ...
        <span v-if="Date.now() <= new Date(faire.faire_end_dt).getTime()">
          <div class="mat-ticketing" :id="'tickets'+entry.project_id" v-if="entry.status=='Accepted' && entry.tickets.length">
            <b-row><b-col>
