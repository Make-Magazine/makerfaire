   ...
   var vm = new Vue({
      ...
      data: {
         ...
         filterVal: '',
			pastFaires: false,
			types: [
				{name: "Featured", description: "Must See Faires"}, 
				{name: "Mini", description: "Global Community Faires"}, 
				{name: "School", description: "Local School Maker Faires"}
			],
         ...
         searchFilter: function(data) { 
			...
			psFilter: function(data){ 
				if(this.pastFaires == true) {
					...
					this.tableData = this.outputData.filter( function(values) {
						var startDate = new Date(values.event_start_dt);
						if(startDate > oneYearAgo) { // this shows 365 days of faires, to show more just return all values
							return values;
						}
					});
				}
