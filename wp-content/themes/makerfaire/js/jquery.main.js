...
function makeStyle() {
   ...
   */
  var side_elem = jQuery('.navbar-nav > .whats-it-about .drop-holder .drop .column ul.sub-menu li.side');
  if (side_elem.length > 0) {
    jQuery(".nav .whats-it-about .dinamic-content").append('<ul></ul>');
    side_elem.each(function(index, val) {
      jQuery(".nav .whats-it-about .dinamic-content ul").append(val);
    });
  }
  /**
   * This splits into 2 halfs the menu links
   */
  var categ_item_li = jQuery('.navbar-nav > .whats-it-about .drop-holder .drop .column ul.sub-menu li'); // select the elements of the menu
  ...
  jQuery('.navbar-nav > .around-the-world-item .drop-holder .drop .right').prepend('<h2>GET STARTED</h2>');

  /**
   * The Around the World menu item (menu's 4th child)
   */
  side_elem = jQuery('.navbar-nav > .around-the-world-item .drop-holder .drop .column ul.sub-menu li.side'); // select the elements of the menu that has class: side
  if (side_elem.length > 0) {
    jQuery(".nav .around-the-world-item .dinamic-content").append('<ul></ul>');
    side_elem.each(function(index, val) {
      jQuery(".nav .around-the-world-item .dinamic-content ul").append(val);
    });
  }

  var parent = jQuery('.navbar-nav > .around-the-world-item .drop-holder .drop .column ul.sub-menu li.child');
  parent.wrapAll("<li class='right'><ul class='show' />"); // (step2)
  jQuery('.navbar-nav > .around-the-world-item .drop-holder .drop .column ul.sub-menu > li.item').addClass('left');
}
//end of menu
