...
function build_makershed_feed_table($collection = "not-set") {
    global $wpdb;
    $wpdb->show_errors();
    ...
    $products = 'products(first:12,sortKey:BEST_SELLING){edges{node{title,status,handle,variants(first: 1){edges{node{price,availableForSale}}},images(first:1){edges{node{url}}}}}}';
...
function makershedOutput($collection = "new-arrivals", $amount = "4") {
    global $wpdb;
    $wpdb->show_errors();
    ...
    if(!empty($makershedProducts)) {
    	...
    	$term = get_term_by('slug', $makershedProducts[0]->collection, 'makershed_collection');
    	
    	// Start building the html output
    	$makershedOutput = '<div class="related-makershed-wrapper amount-'.$amount.'">';
    	$makershedOutput .= '<h3>From the Shed: ' . $term->name  . '</h3>';
