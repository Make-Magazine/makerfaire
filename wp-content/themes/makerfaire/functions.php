...
function load_admin_scripts() {
    ...
    wp_enqueue_script('jquery-accordion', 'https://makerfaire.com/wp-includes/js/jquery/ui/accordion.min.js', array('jquery'), null);
    ...
    wp_enqueue_style('jquery-accordion', get_stylesheet_directory_uri() . '/css/jquery-accordion.min.css'); 
...
function angular_scripts() {
    if (is_page('ribbons') || is_page_template('page-schedule.php') || is_page_template('page-meet-the-makers.php')) {
        ...
        if (is_page_template('page-ribbons.php')) {
...
function validate_url($url) {
    if (preg_match('/^(http|https):\\/\\/[a-z0-9_]+([\\-\\.]{1}[a-z_0-9]+)*\\.[_a-z]{2,5}' . '((:[0-9]{1,5})?\\/.*)?$/i', $url)) {
        ...
        $url = (string) str_replace($path, implode('/', $encoded_path), $url ?? "");
...
function gf_wprocket()  {
    ...
    // also cache bust to get rid of gravity form cookies
    if(class_exists('GW_Cache_Buster')) {
